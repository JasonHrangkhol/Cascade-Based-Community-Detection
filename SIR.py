...
#     #first we remove white spaces from the beginning and end using strip() 
#     #after that we split the string into a list of string after breaking the string using the \t character 
#     # then every string is converted into int using map(int,list)    
#     item = list(map(int,l.strip().split("\t")))
#     clusters[item[1]].add(item[0])

# is considering only those clusters whose size is greater than mincluster
# clusters = [clusters[ele] for ele in clusters if len(clusters[ele])>mincluster]
...
while True:
    ...
    if len(cascade)>= minsize:#checking if the cascade size >= minimum size of cascade
        ...
        #out = ["#",lambda_infect,_iter,len(cascade)-2,len(seen_modules),total_nodes_counter,len(total_seen_nodes),len(total_seen_edges)]
